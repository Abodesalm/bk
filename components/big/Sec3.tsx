...
export default function Sec3({
  ...
  const data = the_data
    ...
    ?.content.find((sub) => sub?.id === ch2)?.content;
  return (
    <div
    ...
    >
      <div className="flex flex-row gap-4 justify-center items-center">
        ...
        <h1 className="text-xl font-bold">
          {ch1 === 3 ? (ch2 === 1 ? title : 'اختر السنة الدراسية') : title}
        </h1>
      </div>
      <div className="flex flex-row md:flex-col flex-nowrap justify-around">
        {data?.map((item) => (
          <button
            ...
            className={`border-2 hover:text-indigo-600 hover:border-indigo-600 hover:-translate-y-1 transition-all duration-150 py-4 px-8 rounded-2xl m-2 font-[500] ${ch3 === item.id ? ' text-white -translate-y-1 hover:text-[#FFF_!important] border-[0] drop-shadow-lg shadow-indigo-400' : ''}`}
          >
            {item.name}
          </button>
        ))}
